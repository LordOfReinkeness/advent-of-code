...
class Maze:
	...
	def map_print(self):
		...
		print('Current cost: {}, On Path Id {}'.format(self.cost, self.id))
	...
	def move(self):
		...
		# If goal reached done (no further movement nessesary)
		if self.arena_map[self.position[1]][self.position[0]] == 'E':
			return True, [self]

		# add current step to map
		self.arena_map[self.position[1]][self.position[0]] = rotations[self.rotation]

		if do_debug:
			self.map_print()

		# If geting to a previously visited field
		# 1. chek if the path is better than the one perviously here
		if heat_map[self.position[1]][self.position[0]] > self.cost:
			heat_map[self.position[1]][self.position[0]] = self.cost
		# 2. Check if we have found a path as good as ther previous
		elif self.cost == heat_map[self.position[1]][self.position[0]]:
			print("[{},{}]: Been here before: {}".format(self.position[0], self.position[1], self))
		# 3. The current path is worse than the one befor: abort this line
		else:
			return False, []
...
def main(data):
	...
	# Your challenges for AOC 2024 day 16 part 2 goes here
	...
	while True:
		...
		if done:
			completetd_paths.append(new_paths[0])
			break
	...
	completetd_paths[0].map_print()

	best_cost = completetd_paths[0].cost
	i = 0
	while paths[i].cost == best_cost:
		#paths[i].map_print()
		i =+ 1
